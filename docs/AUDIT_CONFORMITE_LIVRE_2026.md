...
| B9 — Santé technique | **8,0** | **369/369 tests passés** (dont 23 garde-fous d'intégrité du corpus) ; **0 rouge** — les 7 échecs ont été corrigés (voir le paragraphe « Tests rouges » ci-dessous) ; `tsc --noEmit` propre, `npm run build` OK |
| **VOLET B — moyenne pondérée** | **7,0/10** *(5,1 avant Sprint 0 ; 5,4 après ; 5,6 après le lot 2 ; 5,9 après le lot 3 ; 6,9 après le lot 5)* | Le défaut critique du produit est corrigé. Le plafond est désormais fixé par **B3/B4 — la forme d'évaluation (QCM) reste désalignée du BAC réel** |

*Pondération : B2 compte triple (feedback = cœur de la promesse « tuteur »), B4 et B7 comptent double ; dénominateur 13. Calcul vérifié : (7,5 + 8,0×3 + 4,0 + 3,5×2 + 6,5 + 7,5 + 9,0×2 + 8,5 + 8,0) / 13 = 91,0 / 13 = **7,0**. Note : les révisions précédentes affichaient 7,1 pour une somme qui donnait en réalité 6,92 — l'écart d'arrondi est corrigé ici.*
...
| 27 | **Analyse documentaire : 9 exercices exploitables sur 15** (6 avant le lot 3). Couverture réelle **7 unités sur 11**, tous dispositifs confondus ; **U7, U8, U10 et U11 sans aucun document**. Les 6 exercices restants exigent une **vraie image** (schéma, immunodiffusion, électrophorèse) | B4 | 🟠 Majeur | Moyen-élevé | **P1** — *partiel : `4a5c0f5`* |
...
**7 unités sur 11.** Les trous ne sont pas en géologie mais sur **U7 (التنفس/التخمر), U8 (الحصيلة الطاقوية), U10 (بنية الكرة الأرضية) et U11 (البنيات الجيولوجية)** — quatre unités qui totalisent pourtant **196 QCM**. U11 est la plus dotée du corpus (61 QCM) et n'a aucun document.
