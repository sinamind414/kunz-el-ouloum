...
| B9 — Santé technique | **8,0** | **436/436 tests passés** (dont 23 garde-fous d'intégrité du corpus) ; **0 rouge** — les 7 échecs ont été corrigés (voir le paragraphe « Tests rouges » ci-dessous) ; `tsc --noEmit` propre, `npm run build` OK |
...
**Bilan du lot.** 4 exercices, 4 documents, 10 contextes, 1 bug critique de correction, 6 gabarits assainis, 3 `docType` corrigés. **436 tests verts sur 436** (contre 380), `tsc` rc=0, build OK. Constat **#28** ouvert et soldé dans le même lot ; **#27** ramené de Majeur à Modéré et de P1 à P2 — il ne reste que les 6 documents exigeant une vraie image.
