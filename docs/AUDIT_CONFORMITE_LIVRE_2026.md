...
| B3 — Alignement au format d'épreuve | **4,0** *(décision arrêtée, note inchangée)* | Corpus engendré par 10 gabarits (97,2 %) ; **aucun QCM au BAC réel**. Cette note était restée **en suspens** depuis la révision `e31f9b6`, dans l'attente d'une mesure du seul dispositif susceptible de la relever : les 19 exercices d'analyse documentaire. La mesure a été faite, et elle **confirme le 4,0 au lieu de le relever**. Trois observables. (1) **Volume** : 19 exercices et 43 questions guidées face à **508 QCM** — le format conforme à l'épreuve pèse 8 % des items, et **13 exercices seulement** sont exploitables, les 6 autres affichant « هذه الوثيقة غير جاهزة بعد ». (2) **Structure** : chaque exercice porte **exactement un document** et est noté **sur 20 isolément**, quand l'épreuve algérienne se compose de deux parties indépendantes — 15 points, deux exercices, jusqu'à 2 documents chacun ; puis 5 points d'intégration, jusqu'à 3 documents — portant sur des **domaines différents**. **Aucun des 19 exercices ne croise deux domaines**, et aucun écran ne les compose en une épreuve : c'est #60 vu depuis la donnée et non plus depuis l'interface. (3) **Couverture** : l'unité 9 n'a **aucun** exercice documentaire, et 5 des 19 se concentrent sur l'unité 5. Les correctifs #65 à #67 de ce tour améliorent l'**honnêteté** de cet écran (B10) et la lisibilité de son barème (B2) — ils ne changent rien à son **alignement** : montrer honnêtement une grille sur 20 pour un document isolé ne rapproche pas de l'épreuve. **La note ne bougera que lorsqu'un mode examen à deux parties et deux domaines existera**, ce que les 19 exercices rendent désormais possible mais qu'aucun code ne fait |
...
| 63 | **Le travail de l'enseignant restait enfermé dans l'écran où il l'avait saisi : sa revue n'atteignait jamais la leçon.** Le panneau « وضع الأستاذ » soumet **60 items** à l'arbitrage (5 cartes de survie, 13 résumés de leçon, 42 contextes de document), mais **seules les 5 cartes** disposaient d'un affichage côté élève. **Sonde exécutée** : après publication d'une revue signée « أ. بن يوسف » sur un résumé de leçon, `getAllEditorialItems()` retourne bien `reviewed: true`, tandis que `MissionBanner` — affiché en tête de chaque leçon — continue d'afficher « **شرح Kunz** », le libellé des contenus non relus. Cause : le libellé était calculé sur `summary.status`, valeur **figée dans la donnée** (`adaptation_pedagogique` pour les 13 résumés, `reviewedBy` absent partout), sans jamais consulter l'override. Le défaut est le miroir exact de #59 : là, l'application **affichait une caution qu'elle n'avait pas** ; ici, elle **taisait une relecture qu'elle avait**. Les deux abîment le même contrat, et celui-ci décourage la seule pratique qui pourrait solder #21 — faire relire le contenu par un enseignant en exercice — puisque le relecteur ne voit aucun effet à son travail. **Traité** : `MissionBanner` lit la revue effective, filtrée par la porte de plausibilité de #59, et l'annonce avec la même honnêteté — « **مراجعة محلية على هذا الجهاز (nom) — غير موثقة** », sans jamais emprunter « مصدر موثّق », réservé à la donnée livrée. **4 mutations sur 4 tuées.** **Volet « 42 documents » soldé depuis** (`9ed6504`) : un badge de provenance est affiché dans l'exercice documentaire, et la règle de confiance — recopiée jusque-là dans deux composants — est devenue unique (voir #69). L'instruction de ce volet a par ailleurs mis au jour un défaut plus grave que celui qu'elle devait corriger : **#68**. | Application | 🟠 Majeur | S | ✅ **Résolu** — `082a9aa` puis `9ed6504` (16 tests, 9 mutations tuées) |
| 64 | **Le bouton « signaler une erreur de correction » promettait une équipe et un entraînement d'algorithme — et n'écrivait rien.** Sur la feuille de résultat de l'analyse documentaire, `أبلغ عن خطأ في التصحيح` ouvrait une `alert()` : « تم تسجيل إشعارك. **سيتولى فريقنا** مراجعة هذه الإجابة **وتدريب الخوارزمية** لتحسين التصحيح » (*votre signalement a été enregistré ; notre équipe relira cette réponse et entraînera l'algorithme*). **Sonde exécutée** : clic sur le bouton après correction ⇒ les clés de `localStorage` sont **identiques avant et après** (`kunz_learning_errors_v1`, `kunz_storage_meta_v1`), et le compteur d'appels à `logEvent` reste à **0**. Rien n'est écrit, rien n'est mis en file, rien n'est envoyé. Or l'application est **hors ligne par conception** : il n'existe aucune équipe joignable, et le correcteur est un moteur de règles local — `ValidationEngine`, 12 lois — qui **ne s'entraîne pas**. La promesse était donc fausse deux fois, et la phrase la plus grave est « تم تسجيل إشعارك » : l'élève qui a raison contre le correcteur croit avoir été entendu et n'en reparlera pas. C'est la famille #56/#57/#59 — l'application affirme plus qu'elle ne fait. | Application | 🟠 Majeur | S | ✅ **Résolu** (`9bb7117`) — `correctionFeedbackService` persiste réellement le désaccord sur l'appareil (file bornée à 50, les plus récents conservés) ; la confirmation n'apparaît **que si l'écriture a eu lieu**, et annonce « حُفظ اعتراضك على هذا الجهاز فقط — لا يُرسل إلى أي جهة » en invitant à le montrer à un enseignant ; en cas d'échec de stockage l'échec est **avoué** |
| 65 | **Le score de forme s'affichait « 20/20 », homonyme de la note d'épreuve.** `ValidationEngine.ts:129` fixe `maxScore = ctx.maxScore ?? 20` ; aucun exercice documentaire ne renseignant `maxScore`, **les 43 questions guidées** affichent une note sur 20. Or l'épreuve algérienne est elle aussi notée sur 20, et le barème d'entraînement de chaque exercice totalise **exactement 20 points**. Trois grandeurs distinctes portaient donc le même habillage, alors que le score affiché ne mesure que la **conformité méthodologique** — la forme, jamais le fond scientifique, comme #41 l'avait déjà établi. Un élève lisant « 20/20 » après une réponse partielle en conclut qu'il aurait la note maximale au BAC. | Application | 🟠 Majeur | S | ✅ **Résolu** (`9bb7117`) — la note est préfixée `منهجية:` et nomme ce qu'elle mesure |
| 66 | **76 critères de barème existaient en donnée et n'étaient affichés nulle part.** Chaque exercice documentaire porte un `grilleEntrainement` de 4 critères pondérés totalisant 20 points (`تحديد PPM (لا PPSE)` 4 ن, `علاقة كلما…كلما…` 6 ن, `تفسير جزيئي وخلوي` 6 ن, `وحدات صحيحة` 4 ن), soit **76 critères sur les 19 exercices**. `grep` sur le dépôt : le champ n'a **aucun consommateur hors du fichier de données et d'un test d'intégrité**. **Sonde exécutée** sur l'écran monté : aucun des 4 critères n'apparaît, ni avant ni après la tentative. L'élève recevait donc une note sans jamais savoir sur quoi elle portait — alors que la donnée qui le lui dirait était déjà écrite. C'est le même motif que #63 : un contenu pédagogique réel, produit et maintenu, qui n'atteint pas son destinataire. | Application | 🟠 Majeur | S | ✅ **Résolu** (`9bb7117`) — grille affichée **après** la tentative (jamais avant : elle souffle la réponse) comme instrument d'auto-correction assumé, l'application ne prétendant pas évaluer ces critères automatiquement |
| 67 | **L'avertissement anti-confusion le plus important de l'écran était rédigé en français.** `LABEL_TRAINING` (`scoring.ts:14`) vaut « *Grille d'entraînement Kunz — n'est pas le barème officiel du sujet BAC* » et était affiché tel quel, en 10 px gris clair, sous la note — à un élève **arabophone**, dans une interface intégralement en arabe et en RTL. La seule phrase qui empêche de confondre la note de l'application avec une note officielle était donc illisible pour celui qu'elle protège. Un test d'intégrité (`documentAnalysis.integrity.test.ts:136`) **vérifiait sa présence** dans la donnée, ce qui donnait l'assurance que le garde-fou tenait : il vérifiait une chaîne, pas une lecture. | Application | 🟠 Majeur | S | ✅ **Résolu** (`9bb7117`) — traduit : « سلّم تدريبي من إعداد التطبيق — ليس سلّم التنقيط الرسمي لموضوع البكالوريا » ; test durci ⇒ **aucune lettre latine** admise dans le message affiché |
| 68 | **Relire une question en certifiait trois : la revue de l'enseignant débordait sur du contenu qu'il n'avait pas lu.** En instruisant le volet ouvert de #63, la première mesure a porté non sur l'affichage mais sur l'**identité** des items arbitrés — et c'est là qu'était le défaut. Les 42 contextes de document ne portent que **24 `exerciseId` distincts** : **13 exercices comptent 2 à 3 questions**, chacune avec son objectif, son vocabulaire et sa preuve attendue propres. Or la revue était indexée sur l'`exerciseId` **seul**. **Sonde exécutée** : après une unique publication signée sur `sarin_gb_double`, `getAllEditorialItems()` retourne **3 items relus** — « تحديد آلية تأثير السارين » mais aussi « اقتراح فرضية جزيئية » et « المصادقة على الفرضية », deux contenus distincts que le relecteur n'avait jamais vus. Le panneau affichait de surcroît ces trois lignes sous le **même libellé**, si bien que l'enseignant ne pouvait pas savoir laquelle il publiait, et React recevait trois enfants de même `key`. La gravité tient à ce que le défaut **fabrique une caution** : c'est #59 sous une autre forme — non plus une signature inventée par l'élève, mais une signature authentique **étendue** par le code à ce qu'elle ne couvre pas. La clé est désormais le **couple `(exerciseId, questionId)`**, celle-là même qu'utilise le résolveur de contenu `getDocumentPracticeContext` : une revue doit désigner exactement ce qui a été relu. | Application | 🔴 Critique | S | ✅ **Résolu** (`9ed6504`) — 42 identifiants distincts ; le libellé du panneau porte le rang de la question ; **mutation U** — retour à l'`exerciseId` seul — **rejouée et tuée** (4 rouges) |
| 69 | **La règle qui décide si un contenu est « fiable » existait en deux copies divergentes, et manquait à la troisième surface.** `MissionBanner` (#63) et `SurvivalCardView` (#59) appliquaient chacun leur propre version de la doctrine « une revue locale s'annonce locale » ; les contextes de document n'en avaient **aucune**. Trois copies d'une règle de confiance, c'est trois occasions de la desserrer d'un seul côté sans que rien ne le signale. Un second point, mesuré : **31 des 42 contextes ne portent aucun `sourceStatus`** — un champ absent était donc à un `??` près d'être traité comme une caution. La règle est extraite dans `reviewBadgeService`, avec trois invariants verrouillés : un statut **absent ne vaut pas caution**, « مصدر موثّق » reste **réservé à la donnée livrée**, une revue implausible (nom blanc, date de l'an 3000) **n'est pas affichée**. Au passage, le libellé par défaut « **شرح Kunz** » — un mot latin dans une interface arabophone, exactement le défaut #67 — est remplacé par « من إعداد التطبيق — لم يراجعه أستاذ ». **Trois assertions existantes ont été repointées sur la constante partagée** plutôt que réécrites : leur intention est inchangée et la comparaison reste stricte (`toBe`). | Application | 🟠 Majeur | S | ✅ **Résolu** (`9ed6504`) — **mutations V, W, Y, Z tuées** : statut absent traité comme fiable, revue locale empruntant « مصدر موثّق », badge non rendu, retour au libellé latin |

> **Note de lecture du registre.** Il compte **67 constats** numérotés de 1 à 69 : les numéros **#34 et #35 n'ont jamais été attribués** (saut de numérotation lors du lot 3, et non lignes supprimées — vérifié dans l'historique Git). Répartition actuelle, **recomptée sur les cellules du tableau** et non reportée de la version précédente : **46 résolus**, **4 partiellement résolus** (#7, #27, #33, #47), **1 invalidé** (#4), **16 ouverts** (#2, #5, #6, #8, #10, #13, #14, #15, #16, #18, #19, #20, #21, #23, #31, #60). Le constat **#12** porte sa mention « RÉSOLU » dans la colonne *Gravité* et non dans la colonne *Statut* : un comptage automatique le classe à tort parmi les ouverts, et c'est en vérifiant cet écart plutôt qu'en publiant le total brut que le chiffre ci-dessus a été arrêté. Un pipe littéral figurant dans une cellule doit être **échappé par une barre oblique inverse**, faute de quoi la ligne se scinde et le tableau devient illisible : six lignes présentaient ce défaut et ont été corrigées. Point vérifié contre le moteur de rendu de GitHub (endpoint `/markdown`, mode `gfm`) plutôt que supposé — à l'intérieur d'un code span, l'entité HTML `&#124;` s'affiche telle quelle et ne convient pas. L'intégrité du tableau a donc été contrôlée sur le HTML réellement produit, qui rend bien **67 lignes de 6 cellules** (recomptées sur le tableau après l'ajout de #68 et #69, et non reportées de la version précédente : le comptage est refait sur les cellules à chaque révision, en isolant le registre des autres tableaux du rapport — les inclure faisait apparaître 82 lignes et un total faux).
...
### 3.12 La feuille de correction : quatre défauts sur l'écran le plus proche du BAC réel

Après le circuit éditorial, il restait à regarder l'écran vers lequel tout le reste de l'audit pointe. Depuis #60, la conclusion s'impose : le QCM n'est pas à l'examen, l'**analyse documentaire** l'est. Les 19 exercices documentaires sont donc la partie de l'application la plus proche de l'épreuve — et **aucun test ne les couvrait**. Quatre défauts y coexistaient, tous sur la même feuille de résultat.

**Un bouton qui promet une équipe, dans une application hors ligne.** Sous la correction, `أبلغ عن خطأ في التصحيح` affichait : « votre signalement a été enregistré ; **notre équipe** relira cette réponse et **entraînera l'algorithme** ». J'ai cliqué et mesuré : les clés de `localStorage` sont identiques avant et après, le compteur de télémétrie reste à zéro. Rien n'est écrit, rien n'est mis en file. Il n'y a ni équipe joignable — l'application est hors ligne par conception — ni algorithme qui s'entraîne : le correcteur est un moteur de 12 règles. Le mot le plus grave est « enregistré » : **l'élève qui a raison contre le correcteur croit avoir été entendu, et se tait.** Le geste avait pourtant une valeur réelle à portée de main ; il est désormais persisté sur l'appareil, avec une confirmation qui n'apparaît **que si l'écriture a eu lieu**, et qui dit exactement où va l'information — nulle part ailleurs que sur ce téléphone, à montrer à un enseignant.

**Trois grandeurs sur 20, un seul habillage.** Le verdict affichait « 20/20 ». Or le maximum du moteur vaut 20 *par défaut* (`ctx.maxScore ?? 20`), l'épreuve algérienne est notée sur 20, et le barème d'entraînement de chaque exercice totalise 20 points. Trois choses différentes, une seule apparence — alors que la note affichée ne mesure que la **conformité méthodologique**, jamais le fond scientifique, ce que #41 avait déjà établi. Un élève qui lit « 20/20 » sur une réponse partielle en conclut qu'il aurait la note maximale au BAC.

**Le barème existait, et personne ne le voyait.** Chaque exercice porte quatre critères pondérés — identifier le PPM et non le PPSE (4 points), la relation « plus… plus… » (6), l'interprétation moléculaire et cellulaire (6), les unités (4). **76 critères sur les 19 exercices**, rédigés, maintenus, et sans aucun consommateur hors d'un test d'intégrité. L'élève recevait une note sans savoir sur quoi elle portait, alors que la donnée qui le lui aurait dit était déjà écrite. C'est le motif de #63 une seconde fois, et il commence à ressembler à une signature du produit : **le contenu pédagogique est produit avec sérieux, puis n'atteint pas son destinataire.** La grille est maintenant montrée après la tentative — jamais avant, elle souffle la réponse — comme instrument d'auto-correction assumé : l'application ne sait pas évaluer ces critères, elle ne prétend donc pas le faire.

**Et le garde-fou était écrit dans une langue que l'élève ne lit pas.** La phrase qui empêche de confondre la note de l'application avec une note officielle — « n'est pas le barème officiel du sujet BAC » — s'affichait **en français**, en 10 px gris, dans une interface intégralement arabe et RTL. Un test vérifiait sa présence, ce qui donnait toute assurance que le garde-fou tenait ; il vérifiait une **chaîne de caractères**, pas une **lecture**. C'est la leçon la plus transposable de cette section, et elle vaut au-delà de cet écran : un avertissement se mesure à ce que son destinataire en comprend, pas à sa présence dans le DOM. Le test durci interdit désormais toute lettre latine dans ce message.


### 3.13 Le volet ouvert de #63, et ce qu'il cachait

Il restait un volet ouvert : les **42 contextes de document**, majorité des items du circuit éditorial, sans aucune surface d'affichage. Je l'ai instruit en croyant n'avoir qu'un écran à brancher. La première mesure a porté ailleurs — sur l'**identité** des items arbitrés — et elle a livré le constat le plus grave de cette fin d'audit.

**Une signature authentique, étendue par le code à ce qu'elle ne couvrait pas.** Les 42 contextes ne portent que **24 `exerciseId` distincts** : treize exercices comptent deux à trois questions, chacune avec son objectif et sa preuve attendue. La revue, elle, était indexée sur l'`exerciseId` seul. J'ai publié une relecture signée sur `sarin_gb_double` et compté : **trois items marqués relus**, dont « proposer une hypothèse moléculaire » et « valider l'hypothèse », que le relecteur n'avait jamais ouverts. Le panneau affichait de surcroît ces trois lignes sous un libellé identique — l'enseignant ne pouvait pas savoir laquelle il publiait.

Ce défaut est **#59 retourné**. Là, l'élève pouvait forger une caution qui n'existait pas ; ici, une caution réelle, donnée de bonne foi par un enseignant, est **élargie par le code** à du contenu non lu. Le second cas est plus difficile à détecter, parce que rien dans la chaîne n'est faux : la signature est vraie, la date est vraie, seul le **périmètre** est faux. Et il touche précisément #21, le reproche le plus lourd de l'audit : on ne peut pas demander une validation enseignant à un dispositif qui étend les validations qu'il reçoit. La clé est désormais le couple `(exerciseId, questionId)` — celle qu'utilisait déjà le résolveur de contenu. **La bonne clé existait à trois lignes de là ; c'est la revue qui ne s'en servait pas.**

**Une règle de confiance en deux copies, absente de la troisième surface.** En branchant enfin l'affichage, j'ai trouvé la doctrine de #59 recopiée dans `MissionBanner` et dans `SurvivalCardView`, et nulle part pour les documents. Trois copies d'une règle qui décide si un contenu peut se dire fiable, c'est trois occasions de la desserrer d'un seul côté. Elle est maintenant unique, avec trois invariants tenus par des mutations : un `sourceStatus` **absent — le cas de 31 contextes sur 42 —** ne vaut pas caution ; « مصدر موثّق » reste réservé à la donnée livrée ; une revue implausible ne s'affiche pas. Le libellé par défaut, « شرح Kunz », mêlait par ailleurs un mot latin à une interface arabophone : le défaut #67, rencontré une seconde fois sur un autre écran, ce qui confirme qu'il était **systémique et non local**.

**Trois tests existants sont passés au rouge, et ils avaient raison de le faire.** Ils comparaient le libellé à la chaîne littérale « شرح Kunz ». Leur intention — aucune revue non plausible ne doit modifier l'affichage — restait exacte ; seule la formulation changeait. Je les ai **repointés sur la constante partagée en gardant le `toBe` strict**, plutôt que de les assouplir en `toContain` : la distinction entre mettre un test à jour et le désarmer tient tout entière dans ce choix, et c'est la même règle que celle appliquée aux sept rouges du lot 1 et aux cinq fixtures sacs-de-mots du Sprint 1.

**Ce que ce dernier volet dit de l'ensemble.** Un volet que je pensais cosmétique — « brancher un affichage » — a livré un critique. C'est la cinquième fois dans cet audit qu'une surface ouverte pour une raison mineure en révèle une majeure, et cela vaut comme conclusion de méthode : **le nombre de défauts trouvés dans ce produit continue de mesurer l'étendue de ce qui a été regardé, pas l'état de ce qui reste.**
