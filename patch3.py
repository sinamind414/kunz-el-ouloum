...
# We want to replace from: {activeLessonsForSelectedUnit.length > 0 && (
# To the end of the second <section> block that maps sortedLessonsForUnit.

start_str = "{activeLessonsForSelectedUnit.length > 0 && ("
end_str = "            </section>"

start_idx = content.find(start_str)
# Find the second </section> after start_idx
first_section_end = content.find(end_str, start_idx) + len(end_str)
second_section_end = content.find(end_str, first_section_end) + len(end_str)

if start_idx != -1 and second_section_end != -1:
    content = content[:start_idx] + """<section className="space-y-2.5 rounded-2xl bg-transparent">
            ...
            </section>""" + content[second_section_end:]
