// okachaQuality.ts — filtre OCR d'affichage du بنك الحفظ (lot A+D, 2026-09-24).
//
// Contexte (audit qualité arabe 2026-09-23) : le corpus de LIVRE
// (okacha.ts → okachaEnriched.ts, rendu par OkachaView) contient ~1–2 % de
// déchets scan purs (traits `____`, carrés `■`, filigranes « المتفوف »,
// soupe de lettres) non couverts par les verrous structurels.
//
// RÔLE — UNIQUEMENT l'-affichage (OkachaView) :
//   • scoreDechetOCR / estDechetOCR : masque les blocs score ≥ SEUIL_OCR ;
//   • assainirTexte : retire les marqueurs résiduels des blocs conservés.
//
// INTERDITS :
//   • ne PAS éditer okacha.ts / okachaEnriched.ts (générés) ;
//   • ne PAS toucher au moteur (MethodologyCompilerView, meftahManhajia…) ;
//   • ne PAS réintroduire les 17 signatures OCR purgées ;
//   • lot B (régénération scriptée) pourra appliquer ce score en amont —
//     jamais d'édition manuelle du corpus.
//
 ...
 * Calibré 2026-09-24 sur okachaEnriched (~1 187 blocs) :
 *   • 0 faux positif sur titres/points propres et sur erreurs de lettres
 *     lisibles (« الأحماض الأمنية » reste affiché → corrigé plus tard en lot C) ;
