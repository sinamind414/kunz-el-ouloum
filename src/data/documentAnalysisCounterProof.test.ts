...
import { getDocumentPracticeContext } from './documentPracticeContexts';
...
function verdictAffiche(
  ...
  return (
    validateAnswer(answer, q.ctx).passed &&
...
describe('#40 — contre-épreuve de la surface « analyse de document »', () => {
  it('sens direct : la correction officielle de chaque exercice passe sur toutes ses questions', () => {
    const refusees = QUESTIONS.filter(({ ex, q }) => !noter(ex.correctionAr, q.ctx).reussi).map(
      ({ ex, q }) => `${q.id} (${noter(ex.correctionAr, q.ctx).pourcentage}%)`,
    );
    expect(refusees, `corrections officielles refusées : ${refusees.join(', ')}`).toEqual([]);
  });

  it('sens direct : le gabarit suggéré par l’application est accepté par l’application', () => {
    const refuses = QUESTIONS.filter(({ q }) => q.templateHint && !noter(q.templateHint, q.ctx).reussi).map(
      ({ q }) => q.id,
    );
    expect(refuses, `gabarits refusés : ${refuses.join(', ')}`).toEqual([]);
  });
