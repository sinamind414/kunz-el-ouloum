...
export default function SpeechToTextInput({ placeholder = "اكتب إجابتك هنا أو استخدم الميكروفون", onValidate, expectedKeywords = [], lang = 'ar-DZ' }: SpeechToTextInputProps) {
  ...
  useEffect(() => {
    ...
    if (SpeechRecognitionCtor) {
      ...
      try {
        ...
        // On force la langue arabe avec une locale large (ar-SA est souvent mieux supportée)
        recognition.lang = lang === 'ar-DZ' ? 'ar-SA' : lang; 
  ...
  const toggleListening = () => {
    if (!recognitionRef.current) {
       alert('عذراً، ميزة الإملاء الصوتي غير مدعومة في متصفحك أو تحتاج إلى إذن الميكروفون.');
       return;
    }
    ...
    if (isListening) {
      ...
      try {
        ...
        // Handle cases where start() fails (e.g. permission denied)
        console.warn("Could not start speech recognition", e);
        setIsListening(false);
        alert('حدث خطأ. تأكد من السماح للتطبيق باستخدام الميكروفون في إعدادات متصفحك.');
      }
    }
  };
