...
### V3 — البوصلة (La Boussole) 🧭

النسخة V3 تحوّل التطبيق من «مكتبة محتوى» إلى **مسار تقدّم موجّه**:

- **Focus Engine** (`src/services/focusEngine.ts`) : عند كل فتح، يحدد الوحدة النشطة
  (أول وحدة مفتوحة غير متقنة) والخطوة التالية الوحيدة (درس → QCM → امتحان → معالجة).
- **Mastery Engine** (`src/services/masteryEngine.ts`) : امتحان التحقق «الحارس» —
  10 أسئلة من الوحدة فقط، عتبة النجاح **80%** لفتح الوحدة الموالية. بعد الفشل:
  تشخيص الثغرات + تدريب تصحيحي بأسئلة الأخطاء + إعادة بأسئلة جديدة (أبداً مجرد «أعد المحاولة»).
- **Gating Engine** (`src/services/gatingEngine.ts`) : الوحدات المستقبلية مقفلة،
  والوحدات المفتوحة تبقى دائماً قابلة للوصول. النقر على وحدة مقفلة يستدعي المرشد
  (ليس جداراً صامتاً). **اختبار تشخيصي** يسمح للتلميذ المتقدم بفتح وحدة دون إعادة
  دروسها، و**وضع الأستاذ** (في «تقدمي») يفتح كل الوحدات للاستعمال داخل القسم.
- **Coach proactif** (`src/components/CoachEventModal.tsx`) : يظهر في الأحداث الحاسمة —
  وحدة مقفلة، نجاح (احتفال + كسر القفل)، فشل (خطة معالجة)، عودة بعد غياب ≥ 3 أيام.
- شاشة **مساري** هي نقطة الدخول اليومية : «هدفك الحالي» + زر واحد
  **«أكمل من حيث توقفت»** ينقل التلميذ مباشرة إلى الدرس أو التمرين المناسب.

القاعدة المؤسسة : ***مهمة مرئية واحدة، خطوة تالية واحدة، لا طرق مسدودة.***

> بيانات V3 محفوظة في `kunz_v3_mastery_v1` / `kunz_v3_teacher_unlock` (localStorage)،
> منفصلة عن تقدم V2 — لا يُمس تقدم التلاميذ التجريبيين.
